...
const el = {
  ...
  viewerIdleAnimation: $('viewerIdleAnimation'),
...
function initializeInterfaceLanguage() {
  /*
   * Korean-only mode.
   * Ignore old ?lang=en parameters and any English value saved previously.
   */
  currentLanguage = 'ko';
...
function createViewerFallingBalls() {
  if (!el.fallingBalls || el.fallingBalls.childElementCount > 0) {
    return;
  }
  ...
  for (let index = 0; index < ballCount; index += 1) {
    ...
    el.fallingBalls.appendChild(ball);
  }
}

function showViewerStoppedAnimation() {
  if (!el.viewerIdleAnimation) {
    return;
  }

  createViewerFallingBalls();

  el.viewerIdleAnimation.classList.remove(
    'is-hidden'
  );

  if (el.viewerWaitingText) {
...
function hideViewerStoppedAnimation() {
  if (!el.viewerIdleAnimation) {
    return;
  }

  el.viewerIdleAnimation.classList.add(
    'is-hidden'
  );
...
function initializeApplication() {
  ...
  el.remoteVideo.addEventListener(
    ...
    () => {
      ...
      if (
        el.remoteVideo.videoWidth > 0 &&
        el.remoteVideo.videoHeight > 0
      ) {
        hideViewerStoppedAnimation();
      }
    }
